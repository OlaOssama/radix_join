...
uint64_t
  RadixJoin::thread_num = 0;
uint64_t
  RadixJoin::part_num = 0;
uint64_t
  RadixJoin::parts_per_thread = 0;

RadixJoin::RadixJoin (Relation * innerRelation,
...
RadixJoin::partitioning ()
{
  ...
  for (uint64_t i = 0; i < RadixJoin::thread_num; i++)
      ...
      for (uint64_t p = 0; p < RadixJoin::parts_per_thread; p++)
	{
      ...
      BuildProbe *buildprobe_job = new BuildProbe (innerSizes,
						   outerSizes,
						   innerJobs,
						   outerJobs,
						   i);
